# AGENTS.md — Lya Jewellery

Boutique e-commerce de colliers artisanaux marocains. TanStack Start déployé sur Netlify. Les commandes passent par WhatsApp/Instagram (pas de paiement en ligne intégré). Les produits sont des données statiques.

## Architecture simplifiée

- Pas de base de données : catalogue statique dans `src/data/products.ts`
- Pas de Stripe actif : flux de vente via WhatsApp/Instagram
- Logo parasol en dentelle SVG inline (`src/components/LyaLogo.tsx`)
- Prix en MAD (Dirham marocain)
- Design : beige foncé (#5C3D2E), or (#C9A96E), crème (#FEFAF3)
- Polices : Playfair Display (titres), Cormorant Garamond (texte), Raleway (UI)

## ⚠️ Avant mise en production

- Remplacer `212600000000` par le vrai numéro WhatsApp dans `index.tsx` et `products/$productId.tsx`
- Ajouter de vraies photos produits (actuellement `/placeholder.png`)

## Project Overview (héritage template)
